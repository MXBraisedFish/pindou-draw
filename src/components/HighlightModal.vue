    ...
    <div class="hl-overlay" @click.self="$emit('close')">
      <div class="hl-dialog" @click.stop>
        ...
        <div class="hl-filter">
          ...
          <button class="hl-btn hl-btn-clear" @click="clearAll()">清空</button>
          <button class="hl-btn hl-btn-all" @click="selectAll()">全选</button>
        </div>

        <div class="hl-num-row">
          <label class="hl-num-label">标记序号</label>
          <select v-model="numberMode" class="hl-num-select">
        ...
        <div class="hl-grid">
          ...
          >
            <div class="hl-swatch" :style="{ background: swatchBg(entry) }">
              <span v-if="checkedIds.has(entry.id)" class="hl-check">✓</span>
            </div>
            <span class="hl-id">{{ entry.id }}</span>
          </div>
        </div>

        <div class="hl-actions">
          <span class="hl-count">已选 {{ checkedIds.size }} 色</span>
          <button class="hl-btn hl-btn-primary" @click="apply()">确认高亮</button>
        </div>
      </div>
    </div>
  </Teleport>
...
const checkedIds = ref(new Set(paletteStore.highlightedColorIds))
const search = ref('')
const onlyUsed = ref(true)
const numberMode = ref(paletteStore.highlightNumberMode)
...
function apply() {
  ...
  paletteStore.setHighlightColors(ids)
  paletteStore.highlightActive = ids.length > 0
  paletteStore.highlightNumberMode = numberMode.value
}
</script>

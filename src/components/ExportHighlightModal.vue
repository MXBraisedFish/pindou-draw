  ...
  <Teleport to="body">
    <div class="ehl-overlay" @click.self="$emit('close')">
      <div class="ehl-dialog" @click.stop>
        ...
        <h3>导出高亮配置</h3>
        <p class="ehl-desc">选择需要在导出图中高亮的颜色，对应格子将被边框标记，其余区域以灰色遮罩覆盖。</p>
...
.ehl-desc { margin: 0 0 14px; font-size: 0.82rem; color: #888; }
.ehl-filter {
...
.ehl-search:focus { border-color: #f59e0b; }
.ehl-check-label {
...
.ehl-btn:hover { background: #e8e8e8; }
.ehl-btn-primary {
  background: #f59e0b; color: #fff; border-color: #f59e0b; font-size: 0.9rem;
  padding: 7px 20px;
}
.ehl-btn-primary:hover { background: #e08f0b; }
.ehl-num-row {
  display: flex; align-items: center; gap: 8px; margin-bottom: 10px;
}
.ehl-num-label { font-size: 0.82rem; color: #555; white-space: nowrap; }
.ehl-num-select {
...
.ehl-cell.checked { border-color: #f59e0b; background: #fff7ed; }
.ehl-swatch {
...
.ehl-count { font-size: 0.88rem; color: #888; }
</style>

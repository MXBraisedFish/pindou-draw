...
function triggerReferencePicker() { elements.referenceImageInput?.click(); }
...
function syncReferenceWindowState() {
  ...
  elements.referenceMinimizeBtn && (
    elements.referenceMinimizeBtn.textContent = state.referenceWindowMinimized ? ICONS.RESTORE : ICONS.MINIMIZE,
    elements.referenceMinimizeBtn.setAttribute('aria-label', state.referenceWindowMinimized ? '还原参考窗' : '最小化参考窗')
  );
...
function handlePointerUp(ev) {
  ...
  ['pointermove', 'pointerup', 'pointercancel'].forEach(event =>
    window.removeEventListener(event, handlePointerMove)
  );

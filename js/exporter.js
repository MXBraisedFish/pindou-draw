...
export function renderExportCanvas(exportCanvas, options = {}) {
  const { includeCodes = false, includeAxes = false, backgroundColor = '#ffffff' }
    = options;
  ...
  exportCtx.fillText(TEXT.exporter.heading, centerX, cursorY);
  ...
  exportCtx.textAlign = 'center';
  exportCtx.textBaseline = 'top';
  exportCtx.fillText(TEXT.exporter.total(totalUsedCells), centerX, cursorY);
  cursorY += totalFont + sectionGap;
  exportCtx.font = `${sectionTitleFont}px ${PIXEL_FONT_FAMILY}`;
  exportCtx.fillText(TEXT.exporter.sectionTitle, centerX, cursorY);
  ...
  if (!usedColors.length) {
    ...
    exportCtx.textBaseline = 'middle';
    exportCtx.fillText(TEXT.exporter.empty, centerX, swatchAreaTop + swatchAreaHeight / 2);
    exportCtx.restore();
  }
  else {
    exportCtx.textBaseline = 'top';
  ...
  exportCtx.fillStyle = '#1f1f1f';
  exportCtx.textBaseline = 'top';
  exportCtx.fillText(TEXT.exporter.paletteLabel(paletteLabel), centerX, cursorY);
  return exportCanvas;
}
...
export function exportImage(options = {}) {
  ...
  const exportOptions = { includeCodes, includeAxes, backgroundColor: effectiveBackground };
  let exportCanvas;
  if (hasHighlight) {
    exportCanvas = document.createElement('canvas');
    exportHighlightManager.renderHighlightedCanvas(exportCanvas, selectedColors, exportOptions);
  } else {
    exportCanvas = document.createElement('canvas');
...
export async function exportToPDF(options = {}) {
  const { includeCodes = false, includeAxes = false, backgroundColor = '#ffffff', filename = `pixel-canvas-${state.width}x${state.height}.pdf` }
    = options;

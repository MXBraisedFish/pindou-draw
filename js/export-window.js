...
export function initializeExportWindow() {
    if (!elements.exportWindow) return;[elements.exportCloseBtn, elements.exportCancelBtn].forEach(btn => btn?.addEventListener('click', () => toggleExportWindow(false)));
...
export function toggleExportWindow(force) {
    ...
    if (next) {
        ...
        exportHighlightManager.updateUsedColors(); // 确保更新使用的颜色列表
    } else {
        resetPreviewState();
    }
    syncExportWindow();
...
function bindExportSettingsEvents() {
    const settings = {
        'exportFormat': value => {
            state.exportSettings.format = value;
            updateExportPreview();
            updateFilenameSuffix();
        },
        'includeCodes': value => {
            state.exportSettings.includeCodes = value === 'true';
            updateExportPreview();
        },
        'includeAxes': value => {
            state.exportSettings.includeAxes = value === 'true';
            updateExportPreview();
        },
        'backgroundType': value => {
            state.exportSettings.backgroundType = value;
            updateExportPreview();
        }
    };
    ...
    elements.exportBackgroundColor?.addEventListener('input', (e) => {
        ...
        this.colorTimeout = setTimeout(updateExportPreview, 100);
    });
...
function initializeExportSettings() {
    state.exportSettings = { format: 'image/png', includeCodes: false, includeAxes: false, backgroundType: 'solid', backgroundColor: '#ffffff', filename: '' };
...
function generateExportPreview() {
    ...
    if (previewContainer) {
        ...
        elements.exportPreviewCanvas.width = Math.max(1, rect.width - 32);
        elements.exportPreviewCanvas.height = Math.max(1, rect.height - 32);
    } else {
...
function updateFilenameSuffix() {
    ...
    switch (state.exportSettings.format) {
        ...
        case 'application/pdf': extension = 'pdf';
            break;
        case 'application/psd': extension = 'psd';
            break;
        case 'application/pd': extension = 'pd';
            break;
...
function handleExportConfirm() {
    ...
    if (settings.format === 'application/pd') {
        ...
        }).then(() => showExportSuccess('PDF')).catch((error) => {

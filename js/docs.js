...
export function initializeDocs() {
  ...
  elements.docsToggleBtn.addEventListener('click', () => toggleDocs());
  elements.docsCloseBtn?.addEventListener('click', () => toggleDocs(false));
...
function syncDocsWindow() {
  ...
  visible && elements.docsWindow.focus?.();
}
function handleKeydown(ev) {
  ev.key === 'Escape' && state.docsVisible && toggleDocs(false);
}
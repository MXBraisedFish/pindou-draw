...
export async function importProjectFile(file) {
  ...
  try {
    ...
    if (isCanvasDirty() && !window.confirm('导入 PD 文件会覆盖当前画布内容，是否继续？')) {
      return;
    }
...
function buildProjectPayload() {
  return {
    ...
    palette: {
      ...
      label: state.currentPaletteLabel || '自定义色卡',
...
function applyProjectPayload(payload) {
  ...
  if (!Object.keys(paletteMap).length) {
    window.alert('PD 文件中的色卡为空，无法导入。');
    return;
  }

  applyPalette(
    paletteMap,
    payload.palette.label || '默认色卡',
    {
      libraryId:
  ...
  if (Number.isFinite(cellSize) && cellSize > 0) {
    createCanvas(payload.canvas.width, payload.canvas.height, { cellSize });
  } else {

...
import { createCanvas, isCanvasDirty, redrawCanvas, validateCanvasSize } from './canvas.js';
import { applyPalette, updateCurrentColorInfo } from './palette.js';

const PROJECT_VERSION = 1;

export function exportProject() {
  if (!state.width || !state.height) { window.alert('请先创建画布后再导出。'); return; }
...
export async function importProjectFile(file) {
  ...
  try {
    ...
    if (!payload) { window.alert('PD 文件内容无效或已损坏。'); return; }
    if (!validateCanvasSize(payload.canvas.width, payload.canvas.height)) { window.alert('PD 文件中的画布尺寸超出限制。'); return; }

    if (isCanvasDirty() && !window.confirm('导入 PD 文件会覆盖当前画布内容，是否继续？')) return;
...
function buildProjectPayload() {
  return {
    ...
    palette: {
      ...
      entries: state.paletteKeys.map(code => {
        const entry = state.palette[code];
        return entry ? { code: entry.code, color: entry.color } : null;
      }).filter(Boolean)
    },
...
function normalizeProjectPayload(raw) {
  ...
  const normalizedEntries = paletteEntries.map(entry => {
    if (!entry || typeof entry.code !== 'string' || typeof entry.color !== 'string') return null;
    const code = entry.code.trim(), color = entry.color.trim();
    return code && color ? { code, color } : null;
  }).filter(Boolean);
  ...
  return {
    ...
    palette: {
      ...
      label: typeof raw.palette?.label === 'string' && raw.palette.label.trim() ? raw.palette.label.trim() : '导入色卡',
...
function applyProjectPayload(payload) {
  const paletteMap = {};
  payload.palette.entries.forEach(entry => { paletteMap[entry.code] = entry.color; });

  if (!Object.keys(paletteMap).length) { window.alert('PD 文件内的色卡为空，无法导入。'); return; }

  applyPalette(paletteMap, payload.palette.label || '导入色卡', {
    libraryId: payload.palette.id && state.paletteLibrary.has(payload.palette.id) ? payload.palette.id : null,
  ...
  for (let y = 0; y < payload.canvas.height; y++) {
    for (let x = 0; x < payload.canvas.width; x++) {
      const code = payload.grid[y]?.[x] ?? null;
      if (!code) { state.grid[y][x] = null; continue; }

      const paletteEntry = state.palette[code];
      if (paletteEntry) state.grid[y][x] = paletteEntry;
      else { state.grid[y][x] = null; missingCodes.add(code); }
    }
  }

  redrawCanvas();
  updateCurrentColorInfo();
  missingCodes.size && window.alert(`部分色号在当前色卡中缺失，已自动忽略：${Array.from(missingCodes).join(', ')}`);
}
...
function clearSelectionLayers() {
  [elements.selectionMaskCtx, elements.selectionContentCtx, elements.selectionOutlineCtx]
    .filter(Boolean)
    .forEach((ctx) => ctx.clearRect(0, 0, ctx.canvas.width, ctx.canvas.height));
}
...
function renderSelectionOutline(selectionBounds, preview) {
  ...
  ctx.save();
  ctx.strokeStyle = preview?.type === 'subtract'
    ? '#ff6b6b'
    : preview?.type === 'move'
      ? '#ffb347'
      : '#4f8dff';
...
function deriveOutlineRect(selectionBounds, preview) {
  if (preview?.type === 'move' && selectionBounds) {
    return {
      x: selectionBounds.x + (preview.offsetX || 0),
      y: selectionBounds.y + (preview.offsetY || 0),
  ...
  if (preview?.rect) {
    const { x1, y1, x2, y2 } = preview.rect;
